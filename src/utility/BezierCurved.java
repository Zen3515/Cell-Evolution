...
public class BezierCurved {
	...
	private static double factorial(int n) {
        if (n < 0) { throw new RuntimeException("n is less than 0"); }
        if (n > 32) { throw new RuntimeException("n is greater than 32"); }
    ...
    /**
     * return list of point that can be vertex
     * @param b - point in format of XYXYXYXYXYXYXYXYXY
     * @param cpts - number of point that we want
     * @return - double[] ***Be ware that it only return the width each point
     */
	public static double[] Bezier2D(double[] b, int cpts) {
		int npts = (b.length) / 2;
		int icount, jcount;
		double step, t;
